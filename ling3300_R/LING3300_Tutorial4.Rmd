...
#### Practice
1. We've seen above how to make a set of rating density plots facetted by question type. Now, see if you can wrangle this Qualtrics data into a format that allows you to create a similarly facetted set of rating density plots for the ratings of familiarity with SSBE, Middlesbrough, and Newcaslte accents. After transforming the data, create the set of facetted plots.
...
RTmeans <- lex %>% 
  group_by(Germanic, gender, workerID) %>%
  summarize(meanRT = mean(RT),
            medianRT = median(RT),
            sdRT = sd(RT),
            count = length(RT))

RTmeansofmeans <- RTmeans %>% 
  group_by(Germanic, gender) %>%
  summarize(meanofmeansRT = mean(meanRT),
            sdofmeansRT = sd(meanRT),
            count = length(meanRT))

RTmeansofmeans$se <- RTmeansofmeans$sdofmeansRT / sqrt(RTmeansofmeans$count)
```
...
```{r eval = T}
RTmeansofmeans$df <- RTmeansofmeans$count - 1
```
...
```{r eval = T}
RTmeansofmeans$tval_lower <- qt(0.025, RTmeansofmeans$df)
RTmeansofmeans$tval_upper <- qt(0.975, RTmeansofmeans$df)
```
...
```{r eval = T}
RTmeansofmeans$margin_of_error_lower <- RTmeansofmeans$se * RTmeansofmeans$tval_lower
RTmeansofmeans$margin_of_error_upper <- RTmeansofmeans$se * RTmeansofmeans$tval_upper
```
...
```{r eval = T}
RTmeansofmeans$lower95CI <- RTmeansofmeans$meanofmeansRT + RTmeansofmeans$margin_of_error_lower
RTmeansofmeans$upper95CI <- RTmeansofmeans$meanofmeansRT + RTmeansofmeans$margin_of_error_upper
```
...
```
So far we've been making ggplots by putting the dataframe name inside ggplot() and all the aesthetics inside the individual plotting functions. But do you see how we have the identical aesthetics aes(x = Germanic, y = meanofmeansRT, color = gender) in both of the plotting functions here? When multiple plotting functions take the same dataframe and aesthetics, we can save some space by sticking the parts of aes() that should be applied to all of the functions into the ggplot() function. The following code therefore accomplishes the same thing as above:
 
```{r eval = T}
ggplot(RTmeansofmeans, aes(x = Germanic, y = meanofmeansRT, color = gender)) +
  geom_point() +
  geom_errorbar(aes(ymin = lower95CI, ymax = upper95CI))
```
This is a bit hard to read, though, because our mean points and error bars are overlapping. We use the command position = position_dodge(width = 0.5) in both the geom_point() and geom_errorbar() functions in order to spread them apart a bit horizontally. (Unfortunately, it doesn't seem like you can put this position command inside the overall ggplot() function even though it's identical for both plotting functions -- you need to specify it in both plotting functions.)
...
```
Our lines of code are getting awfully long, though! Luckily, we can add line returns after not just + signs, but also commas. Here's the same code, but spaced out a bit more to make things more legible:
...
It's also rather common to see line returns after some opening parentheses and before some ending parentheses. Here's another way to write the code which might help you keep track of all the parentheses going on.
...
You can also add labels, or beautify the plot with themes:
