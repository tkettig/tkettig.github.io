...
You can download R [here](https://mirror.csclub.uwaterloo.ca/CRAN/). Once you download it, though, you don't have to ever actually *open* this application, because the R on its own is not very pleasant. If you click on just the R application, it displays what's called a "console" that is a very simple interface that allows you to enter code interactively. You write a line, and it prints out the response, very much like a calculator.

In this course, we'll be using RStudio, a user-friendly interface for running the R language. Once you've downloaded R, you can then [download RStudio](https://docs.posit.co/ide/user/). Like 'naked' R, RStudio also has a console at the bottom left space of your screen. You'll see that each line begins with a '>' and a blinking cursor; this '>' means you should start entering your code there. 
...
Now that you've had a very brief orientation, let's get started! For more detail, I recommend the R and RStudio setup instructions and introductory materials in Jessamyn Schertz's [Talking Numbers](http://individual.utoronto.ca/jschertz/teaching/LIN318_23F/_book/), Nathaniel Phillips' [yarRrr! The Pirate's Guide to R](https://bookdown.org/ndphillips/YaRrr/), and Bodo Winter's [Statistics for Linguists: An Introduction Using R](https://appliedstatisticsforlinguists.org/bwinter_stats_proofs.pdf).
...
```{r eval = FALSE}
mycol <- mandarin[,c(1,5,7)]
```

#### The dollar sign: \$

The dollar sign is incredibly useful when it comes to dealing with columns in a dataset! The dollar sign allows you to name the column that you want to refer to. For example, we can identify and extract the column "maxf0" from the mandarin dataset by calling mandarin$maxf0:
```{r eval = FALSE}
mycol <- mandarin$maxf0
mycol
```

We can also create new columns. For instance, if we wanted to create a column that subtracts the minimum f0 from the maximum f0 of each token to get its pitch range, we could make a new "range" column:
```{r eval = FALSE}
mandarin$range <- mandarin$maxf0 - mandarin$minf0
View(mandarin)
```
...
We can paste strings together using the paste() function. In this example, we'll create a single column called "syllable" that pastes together the "prec" onset consonant with the "vowel" nucleus. To do this, we can paste together the prec and vowel columns with an underscore separating the two elements:
```{r eval = FALSE}
mandarin$syllable <- paste(mandarin$prec, mandarin$vowel, sep="_")
View(mandarin)
```
...
```{r eval = FALSE}
mandarin$syllable <- paste(mandarin$prec, mandarin$vowel, sep="")
View(mandarin)
```
